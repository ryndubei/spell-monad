...
startEval u = do
  -- mask to guarantee the finalizer is added
  mask \restore -> do
    evalAsync <- async $ restore do
      withTrusted u wait
    ...
    addFinalizer evalAsync (restore $ killThread tid) -- finalizer should not depend on the Async itself
